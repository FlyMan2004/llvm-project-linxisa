...
entry:
  %ta = call <1024 x i32> @llvm.linx.tma.tload(ptr %a, i32 12)
  %tb = call <1024 x i32> @llvm.linx.tma.tload(ptr %b, i32 12)
  %out = call <1024 x i32> @llvm.linx.cube.mamulb(<1024 x i32> %ta, <1024 x i32> %tb, i32 4, i32 4, i32 4)
  call void @llvm.linx.tma.tstore(ptr %dst, <1024 x i32> %out, i32 12)
  ret void
}

; CHECK-LABEL: cube_tile:
; CHECK: BSTART.TMA{{.*}}TLOAD
; CHECK: BSTART.TMA{{.*}}TLOAD
; CHECK: BSTART.CUBE{{.*}}MAMULB
; CHECK: BSTART.CUBE{{.*}}ACCCVT
; CHECK: BSTART.TMA{{.*}}TSTORE
